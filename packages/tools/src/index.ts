...
(async () => {
  process.env["APIFY_MEMORY_MBYTES"] = "8192";
  await Actor.init();
  const characters = parseCharacters(6);
  const weapons = parseWeapons(8);
  const artifacts = parseArtifacts(1);
  const promises = [characters, weapons, artifacts];
  const [character, weapon, artifact] = await Promise.all(promises);
  if (character && weapon && artifact) {
